...
UpdateSystem() {
  ...
  read -r -t 60 -p "Do you want to run 'apt-get autoremove' to remove unused packages? [y/N] " response
...
# Replaces the hardcoded Proxmox CT template URL in APLInfo.pm with the mirror.
# Idempotent: no-op once the URL has been replaced. Must be called again after
# any 'apt-get full-upgrade', because a pve-manager package update overwrites
# APLInfo.pm and silently reverts the patch.
PatchAPLInfoURLs() {
  local apl_info_pm="/usr/share/perl5/PVE/APLInfo.pm"
  if grep -q "http://download.proxmox.com" "$apl_info_pm"; then
    sed -i "s|http://download.proxmox.com|${MIRROR_URL}/proxmox|g" "$apl_info_pm"
    LogSuccess "Patched standard CT template URLs in APLInfo.pm."
  else
    LogInfo "APLInfo.pm already patched, skipping."
  fi
}
...
ConfigureTurnKeyTemplates() {
  ...
  # 2. Configure systemd override to patch internal download URLs
...
EOF
  ...
  # 3. Reload systemd and start update service
  LogInfo "Reloading systemd daemon..."
  systemctl daemon-reload
  
  LogInfo "Triggering pve-daily-update.service to fetch and patch templates..."
  if ! systemctl start pve-daily-update.service; then
    LogWarn "pve-daily-update.service failed to start. Continuing anyway."
  fi
...
Main() {
  ...
  # --- 4. Patch Standard CT Template URLs (before pveam update) ---
  LogInfo "Updating URL for standard CT template downloads..."
  PatchAPLInfoURLs

  # --- 5. System Update and Package Management ---
  LogInfo "Updating CT (LXC) templates..."
  pveam update

  UpdateSystem

  # Re-apply APLInfo.pm patch: 'full-upgrade' may have overwritten the file.
  PatchAPLInfoURLs

  read -r -t 60 -p "Do you want to install 'openvswitch-switch' for advanced networking? [y/N] " response
  if [[ "$response" =~ ^([yY][eE][sS]|[yY])$ ]]; then
  ...
  if [[ ! -f "$pveceph_pm_file" ]]; then
    ...
    if [[ "$VERSION_CODENAME" == "bookworm" ]]; then
      ...
      sed -i 's|PVE::Tools::file_set_contents("/etc/apt/sources\.list\.d/ceph\.list", $repolist);|#&|' "$pveceph_pm_file"
      LogSuccess "Patched pveceph.pm for ceph.list."
    elif [[ "$VERSION_CODENAME" == "trixie" ]]; then
      # Patch for PVE 9.x (Trixie) which uses ceph.sources
      sed -i 's|PVE::Tools::file_set_contents("/etc/apt/sources\.list\.d/ceph\.sources", $repo_source);|#&|' "$pveceph_pm_file"
